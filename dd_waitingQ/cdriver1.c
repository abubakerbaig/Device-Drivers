...
static wait_queue_head_t wqR,wqW;
...
static ssize_t sample_read (struct file *filp, char *Ubuff, size_t count, loff_t *offset)   {
    ...
    wait_event_interruptible(wqR,CIRC_CNT(cbuf.head,cbuf.tail,SIZE)>0);
...
static ssize_t sample_write (struct file *filp, const char *Ubuff, size_t count, loff_t *offset)    {
    int i,ret,mini;
    wait_event_interruptible(wqW,CIRC_SPACE(cbuf.head,cbuf.tail,SIZE)>0);
